spec aptos_framework::permissioned_signer {
    ...
    spec module {
        ...
        axiom forall a: GrantedPermissionHandles:
            (forall i in 0..len(a.revoked_handles):
                forall j in 0..len(a.revoked_handles):
    ...
    spec destroy_storable_permissioned_handle(p: StorablePermissionedHandle) {
        ...
        ensures !vector::spec_contains(granted_permissions.revoked_handles, p.permission_addr);
    }
    ...
    spec revoke_permission_handle(s: &signer, permission_addr: address) {
        ...
        let post granted_permissions = global<GrantedPermissionHandles>(master_addr);
        ensures exists<GrantedPermissionHandles>(master_addr) ==>
            vector::spec_contains(granted_permissions.revoked_handles, permission_addr);
    }
