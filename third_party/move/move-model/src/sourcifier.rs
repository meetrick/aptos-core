...
impl<'a> Sourcifier<'a> {
    ...
    pub fn print_fun(&self, fun_id: QualifiedId<FunId>, def: Option<&Exp>) {
        ...
        let tctx = fun_env.get_type_display_ctx_shortcut_module();
        let params = fun_env
            .get_parameters()
            .into_iter()
    ...
    pub fn print_struct(&self, struct_id: QualifiedId<StructId>) {
        ...
        let type_display_ctx = struct_env.get_type_display_ctx_shortcut_module();
...
impl<'a> ExpSourcifier<'a> {
    ...
    pub fn for_fun(parent: &'a Sourcifier<'a>, fun_env: &'a FunctionEnv, exp: &Exp) -> Self {
        let type_display_context = fun_env.get_type_display_ctx_shortcut_module();
    ...
    fn print_constructor<I>(
        ...
        self.parent.print_list(
            ...
            ",",
            "}",
            struct_env.get_fields_optional_variant(*variant).zip(items),
            |(f, i)| {
    ...
    fn module_qualifier(&self, mid: ModuleId) -> String {
        ...
        if self.type_display_context.module_name.as_ref() == Some(module_env.get_name()) {
            ...
            format!(
                "{}::",
                if self.type_display_context.overloaded_modules.contains(&mid) {
